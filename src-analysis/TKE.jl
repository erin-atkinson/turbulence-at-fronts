...
mean_fields = (; u_dfm, v_dfm, w_dfm, b_dfm, u_next_dfm, v_next_dfm, w_next_dfm, b_next_dfm)
...
GSP3D = Field(KernelFunctionOperation{Center, Center, Center}(GSP3D_func, grid, clock, input_fields, mean_fields, sp))
...
TKE3D_fields = (; LSP3D, VSP3D, GSP3D, BFLUX3D, DSP3D, TKE3D, DTKEDt3D)

(LSP, VSP, GSP, BFLUX, DSP, TKE, DTKEDt) = map(dfm, TKE3D_fields)

TKE_fields = (; LSP, VSP, GSP, BFLUX, DSP, TKE, DTKEDt)
