function frontogenesis(
    ...
    v_dfm = timeseries_of(a->filt(a, σ)[:, z_indᶜ:end], joinpath(foldername, "DFM.jld2"), "v_dfm", iterations)
    ...
    b_dfm = timeseries_of(a->filt(a, σ)[:, z_indᶜ:end], joinpath(foldername, "DFM.jld2"), "b_dfm", iterations)
    b_dfm = b_dfm .+ (b_dfm[1, inds[1:1], 1:1] .- b_dfm[:, inds[1:1], 1:1])
    
    ζ_max = maximum(abs, ζ_dfm)
    ...
    ax = Axis(fig[1, 1]; ax_kw..., title="Mixed layer average")
    ...
    ht_kw = (;
        colormap,
        colorrange=(-ζ_max, ζ_max),
        highclip=ζ_max > ht_kw.colorrange[2] ? colormap[end] : nothing,
        lowclip=ζ_max > -ht_kw.colorrange[1] ? colormap[end] : nothing,
        ht_kw...
    )
